...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btn_download').onclick = () => { 
    const data = jm.get_data(), 
          title = (data.data.topic || 'mindmap').replace(/<[^>]+>/g, ''), 
          panAndZoomFnString = enablePanAndZoom.toString(),
          // --- 1. 取得換行函式的字串內容 ---
          wrapFnString = enforceNodeWrapping.toString(); 
...
// === Auto-wrap enforcement helpers ===
function enforceNodeWrapping(jm, maxWidth){
  try{
    const nodes = document.querySelectorAll('jmnode, .jmnode');
    nodes.forEach(el=>{
      el.style.whiteSpace = 'normal';
      el.style.wordBreak = 'break-word';
      el.style.overflowWrap = 'anywhere';
      el.style.maxWidth = (maxWidth||280) + 'px';
      el.style.boxSizing = 'border-box';
      el.style.lineHeight = '1.4';
      el.style.padding = '6px 10px';
    });
    if (jm && jm.layout && typeof jm.layout.layout === 'function') {
      jm.layout.layout();
      if (jm.view && typeof jm.view.show === 'function') jm.view.show();
    }
  }catch(e){ console.warn('wrap enforcement failed:', e); }
}

function setupWrapObserver(jm){
  const container = document.getElementById('jsmind_container');
  if(!container) return;
  let timer=null;
  const obs = new MutationObserver(()=>{
    clearTimeout(timer);
    timer = setTimeout(()=>enforceNodeWrapping(jm,280), 60);
  });
  obs.observe(container, {childList:true, subtree:true, attributes:true, characterData:false});
  // First run
  setTimeout(()=>enforceNodeWrapping(jm,280), 0);
}